...
valid_X = np.floor(np.random.rand(DATA_SIZE, LENGTH) * 2)
...
with tf.Session() as sess:
    ...
    for i in range(EPOCH):
        ...
        valid_loss = sess.run(cost, feed_dict={x_: valid_X, y_: valid_Y})
    ...
    print(sess.run([cost], feed_dict={x_: test_X, y_: test_Y}))
